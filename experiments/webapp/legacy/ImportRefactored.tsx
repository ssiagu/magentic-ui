...
export const ImportRefactored = () => {
    const folderInputRef = useRef<HTMLInputElement>(null);
    
    // Get state and actions from centralized hook
    const {
        runs,
        loading,
        errors,
    ...
    // Extract import-specific state
    const isLoading = loading.importing;
    const error = errors.import;
    const validationError = errors.validation;
    const success = errors.success; // We can repurpose this for success messages
    ...
    const validateAndParseRunData = (data: any): RunData => {
        try {
            ...
            if (validationErrors.length > 0) {
                setError('validation', `Validation failed: ${validationErrors.join(', ')}`);
            ...
            if (err instanceof z.ZodError) {
                ...
                setError('validation', `Schema validation failed: ${zodErrors.join(', ')}`);
    ...
    const handleFolderUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            addRun(runData);
            setError('success', `Successfully imported ${runData.tasks.length} tasks from ${runData.args.length} runs!`);
        } catch (err) {
            const errorMessage = err instanceof Error ? err.message : 'Unknown error';
            setError('import', `Error processing folder: ${errorMessage}`);
        } finally {
            ...
            <div className="bg-white rounded-lg shadow-lg p-6">
                ...
                {validationError && (
                    <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                        ...
                        <div className="text-orange-600 text-sm">{validationError}</div>
                    </div>
                )}
            ...
            {runs.length > 0 && (
                <div className="bg-white rounded-lg shadow-lg p-6">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-lg font-semibold">Imported Data ({runs.length} runs)</h3>
                        <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {runs.map((runData, index) => (
                            <RunCard
                                key={index}
                                run={runData}
                                onDelete={() => handleDeleteRun(index)}
                                showTasks={false}
                            />
                        ))}
                    </div>
