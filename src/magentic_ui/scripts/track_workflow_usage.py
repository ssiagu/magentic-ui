...
def track_workflow_usage(runs_path: str, dataset: str = "WebVoyager"):
    ...
    workflow_scores = defaultdict(list)  # Track scores for trajectories using each workflow
    task_workflow_score_map = defaultdict(lambda: defaultdict(list))  # workflow -> task_id -> scores
    ...
    for task in tasks:
        for task_run in task.task_runs:
            ...
            for workflow_num in workflows:
                ...
                task_workflow_score_map[workflow_num][task.task_id].append(task_run.score)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='WebVoyager', 
    ...
    workflow_counts, task_workflow_map, workflow_scores, task_workflow_score_map = track_workflow_usage(args.runs_path, args.dataset)
    ...
    if not workflow_counts:
        ...
        for workflow_num, count in sorted_workflows:
            ...
            avg_score = sum(workflow_scores[workflow_num]) / len(workflow_scores[workflow_num]) if workflow_scores[workflow_num] else 0
            task_ids = sorted(task_workflow_map[workflow_num])[:3]  # Show first 3 task IDs
            task_ids_str = ", ".join(task_ids)
            if len(task_workflow_map[workflow_num]) > 3:
                task_ids_str += f", ... ({len(task_workflow_map[workflow_num]) - 3} more)"
            
            print(f"[WORKFLOW {workflow_num}]".ljust(15) + f"{count:<10} {avg_score:<12.2f} {num_tasks:<12} {task_ids_str}")
            total_usage += count
        ...
        for workflow_num in sorted(workflow_counts.keys()):
            ...
            print(f"  Average score: {sum(workflow_scores[workflow_num]) / len(workflow_scores[workflow_num]):.2f}")
            ...
            for task_id in sorted(task_workflow_map[workflow_num]):
                ...
                print(f"    - {task_id}: {len(task_scores)} uses, avg score: {avg_task_score:.2f}")
        ...
        if args.output:
                ...
                workflow_stats[workflow_num] = {
                    ...
                    "tasks": {
                        task_id: {
                            ...
                            "average_score": sum(task_workflow_score_map[workflow_num][task_id]) / len(task_workflow_score_map[workflow_num][task_id]) if task_workflow_score_map[workflow_num][task_id] else 0
                        }
            ...
            results = {
                ...
                "total_trajectories": sum(len(task.task_runs) for task in get_tasks_for_system(args.runs_path, args.dataset)),
